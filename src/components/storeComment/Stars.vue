<template>
    <div class="flex gap-2" @mouseleave="resetHoverIndex">
        ...
        ></div>
        <p>{{ starsStore.hint[0] }}
            <label v-if="starsStore.hint[1]" class="p-1 text-sm text-white rounded-full bg-amber-500">{{ starsStore.hint[1] }}</label>
        </p>
    </div>
</template>

<script setup>
import { useStarsStore } from '../../stores/starStore'; // 假設 store 路徑為 src/stores/starsStore
const starsStore = useStarsStore();

const totalStars = 5;

const resetHoverIndex = () => {
  starsStore.setHoverIndex(0); // 使用 Pinia store 的方法
};
